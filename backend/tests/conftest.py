...
@pytest.fixture
def sample_csv_file():
    """Generate a sample CSV file content with valid gold price data."""
    # Create 100 rows of realistic gold price data
...
@pytest.fixture
def small_csv_file():
    """Generate a CSV file with only 50 rows (below 60 row minimum)."""
...
def invalid_csv_file():
    """Generate an invalid CSV file with bad data."""
    csv_content = io.StringIO()
    writer = csv.writer(csv_content)
...
def csv_to_file(csv_content):
    """Helper to convert CSV string content to a file-like object."""
...
def upload_sample_data(client):
    """Helper fixture to upload sample data to a client session."""
    def _upload(csv_content):
        """Upload CSV content and return response."""
        data = {
